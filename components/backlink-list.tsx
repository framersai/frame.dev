 ...
 * @returns Backlink list or empty state message
 * 
 * @example
 * ```tsx
 * const backlinks = (
 ...
 * ```
 */
export default function BacklinkList({ currentPath, files }: BacklinkListProps) {
      ...
      {backlinks.map((b) => {
        ...
        return (
          <li key={b.path}>
            <Link
              href={`?file=${b.path}`}
              className="flex items-start gap-2 px-2 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all group"
            >
              {/* Nice connection node icon */}
              <svg 
